...
export class Board {
  ...
  public figureIsUnderAttack(figureCell: Cell): boolean {
    for (let i = 0; i < this.cells.length; i++) {
      const row = this.cells[i];
      for (let j = 0; j < row.length; j++) {
        const target = row[j];
        ...
        if (
          target.figure &&
          target.isEnemy(figureCell) &&
          target.figure.canMove(figureCell)
        )
          return true;
      }
    }
  ...
  public getKingTarget(color: Colors): Cell | null {
    for (let i = 0; i < this.cells.length; i++) {
      const row = this.cells[i];
      for (let j = 0; j < row.length; j++) {
        const target = row[j];
